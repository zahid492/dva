...
function CopyTable(old)
    local NewTable = {}
    for k, v in pairs(old) do NewTable[k] = v end
    return NewTable
end
...
function CopyArray(old)
    local NewArray = {}
    for k, v in ipairs(old) do NewArray[k] = v end
    return NewArray
end

function print(...) LuaAuxLib.TracePrint(...) end

function match(str, format) return str:match(format) end
function gsub(str, f, s) return str:gsub(f, s) end
...
function QMPlugin.CountStr(str, substr)
    ...
    local iRet, sRet = pcall(function()
        local count = 0
        for k in string.gmatch(str, substr) do count = count + 1 end
        return count
    end)
    if iRet == true then
        return sRet
...
function QMPlugin.LeftDel(str, num)
    sh_init()
    local iRet, sRet = pcall(function() return str:sub(num + 1, -1) end)
    if iRet == true then
        return sRet
    else
        print(sRet)
...
function QMPlugin.RightDel(str, num)
    sh_init()
    local iRet, sRet = pcall(function() return str:sub(1, str:len() - num) end)
    if iRet == true then
        return sRet
    else
        print(sRet)
...
function XmlToTable(xml)
    ...
    local iRet, sRet = pcall(function()
        ...
        for k in xml:gmatch("<node[^>]+/?>") do
            tXml[i] = {}
            for w, y in k:gmatch("([^ ]+)=([^ ]+)") do tXml[i][w] = y end
            i = i + 1
        end
        return tXml
    end)
...
function FindXmlKeyEx(tblXml, tblList, key)
    sh_init()
    local iRet, sRet = pcall(function() end)
    if iRet == true then
        return sRet
    else
        print(sRet)
...
function QMPlugin.RemoveDup(tbl)
    ...
    local iRet, sRet = pcall(function()
        local a = {}
        local b = {}
        for _, v in ipairs(tbl) do a[v] = 0 end
...
function QMPlugin.RndEx(min, max)
    sh_init()
    local iRet, sRet = pcall(function() return math.random(min, max) end)
    if iRet == true then
        return sRet
    else
        print(sRet)
...
function ReadFile(path, isdel)
    local iRet, sRet = pcall(function()
        ...
        if f == null then return "" end
        local ret = f:read("*all")
        f:close()
        if isdel then os.remove(path) end
        return ret
    end)
    if iRet == true then
        return sRet
...
function QMPlugin.GetFileName(path)
    ...
    local iRet, sRet = pcall(function()
        path = "/" .. path
        local ret
        for w in string.gmatch(path, "/([^/]+)") do ret = w end
        return ret
    end)
    if iRet == true then
        return sRet
...
function QMPlugin.WriteFileLine(path, line, str)
    ...
    local iRet, sRet = pcall(function()
        ...
        f:close()
        f = io.open(path, "w")
        for _, v in ipairs(t) do f:write(v .. "\r\n") end
        f:close()
    end)
    if iRet == true then
        return sRet
...
function QMPlugin.FindFileDelete(findpath, filename)
    sh_init()
    execute(string.format("find %s -name '%s' | xargs rm -rf", findpath,
                          filename))
end
...
function QMPlugin.ReadFileBase(path)
    ...
    local iRet, sRet = pcall(function()
        f = io.open(path, "rb")
        if f == null then return null end
        bytes = f:read("*all")
        f:close()
        local key =
...
function Init()
    local iRet, sRet = pcall(function()
        -- 0=白色、1=绿色、2=橙色、3=红色
        if Html.HtmlLevel == nil then Html.HtmlLevel = 1 end
        -- 日志模版
        ...
        if Html.HtmlPath == nil then
            Html.HtmlPath = TempPath() .. "log_" .. os.date("%Y%m%d%H%M") ..
                                ".html"
        end
        -- 日志标题
...
function QMPlugin.AddLog(LogText, Level)
    ...
    local iRet, sRet = pcall(function()
        ...
        if Level ~= nil then tmpLevel = Level end
        -- 单条日志
        tmpData = string.gsub(Html.TempletItem, "{$level}", tmpLevel)
        tmpData = string.gsub(tmpData, "{$data}",
                              os.date("[%H:%M:%S] ") .. LogText)
...
function QMPlugin.AutoSave(mode) if mode then isAutoSave = mode end end
...
function QMPlugin.GetMAC()
    ...
    local iRet, sRet = pcall(function()
        ...
        if FileExist("/sys/class/net/wlan0/address") then
            ...
            s, e = info:find("mac")
            if s then
                _, _, retMac = info:find("%[(%w%w:%w%w:%w%w:%w%w:%w%w:%w%w)%]",
                                         s + 1)
            end
        end
        retMac = retMac or ""
...
function QMPlugin.GetIP()
    ...
    local iRet, sRet = pcall(function()
        local iplist = {
            "http://1212.ip138.com/ic.asp", "http://ip.chinaz.com/getip.aspx"
        }
        local retip = ""
...
function QMPlugin.WeiXinUrl(packagename, url)
    ...
    os.execute(string.format(
                   "am start -n %s/.plugin.webview.ui.tools.WebViewUI -d '%s'",
                   packagename, url))
end
...
function QMPlugin.OpenWeb(url)
    ...
    os.execute(
        string.format("am start -a android.intent.action.VIEW -d " .. url))
end
...
function QMPlugin.GetNavigationBar()
    ...
    local iRet, sRet = pcall(function()
        ...
        local _, _, pos1, pos2 = ret:find(
                                     "NavigationBar.-mFrame=%[%d+,(%d+)%]%[%d+,(%d+)%]")
        if pos1 then
...
function QMPlugin.GetStatusBar()
    ...
    local iRet, sRet = pcall(function()
        local ret = execute("dumpsys window windows")
        local _, _, h, h1 = ret:find(
                                "StatusBar.-mFrame=%[%d+,(%d+)%]%[%d+,(%d+)%]")
        if h ~= nil then
...
function QMPlugin.AppLastUpdateTime(packagename)
    sh_init()
    local UpdateTime =
        execute(string.format("dumpsys package %s ", packagename))
...
function QMPlugin.SetNavigationBar(isDisplay)
    ...
    local iRet, sRet = pcall(function()
        ...
        if retText:find("qemu%.hw%.mainkeys=%w") then
            retText = retText:gsub("qemu%.hw%.mainkeys=%w",
                                   "qemu.hw.mainkeys=" .. mode)
            ReplaceFile(path, "qemu%.hw%.mainkeys=%w",
                        "qemu.hw.mainkeys=" .. mode)
        else
...
function KillClean(pgknamearr)
    ...
    for l in io.lines(localpath) do
        ...
        if string.find(ReadContent, l) then
            ...
            if n == 0 then os.execute("am force-stop " .. l) end
        end
    end
end
...
function QMPlugin.GetHttp(url, t, header)
    ...
    if header == nil then
        os.execute(string.format("curl --connect-timeout %s -o %s '%s' ", t,
                                 path, url))
    else
        os.execute(string.format("curl --connect-timeout %s -H '%s' -o %s '%s'",
                                 t, header, path, url))
    end
...
function QMPlugin.GetHttpFile(url, filepath, header)
    ...
    if header == null then
        ...
        os.execute(string.format("curl -o %s -H '%s' '%s' ", filepath, header,
                                 url))
    end
end

-- UploadFile 上传文件
function QMPlugin.UploadFile(url, filepath, datas, t)
    sh_init()
    local path = TempFile("POST")
    os.execute(string.format(
                   "curl -o '%s' '%s' -X POST -F 'file=@%s' -F '%s' --connect-timeout %s ",
                   path, url, filepath, datas, t))
    result = ReadFile(path, true)
    return result
end
...
function QMPlugin.PostHttp(url, post, t, header)
    ...
    if t == nil then t = 30 end
    if header == nil then
        os.execute(string.format("curl -o %s -d '%s' --connect-timeout %s '%s'",
                                 path, post, t, url))
    else
        os.execute(string.format(
                       "curl -o %s -d '%s' --connect-timeout %s -H '%s' '%s'",
                       path, post, t, header, url))
    end
    result = ReadFile(path, true)
    return result
end
...
function QMPlugin.PostHttpC(url, post, cookie_path, t, header)
    ...
    if header == null then
        os.execute(string.format(
                       "curl -o %s -d '%s' -b '%s' --connect-timeout %s '%s'",
                       path, post, cookie_path, t, url))
    else
        os.execute(string.format(
                       "curl -o %s -d '%s' -b '%s' --connect-timeout %s -H '%s' '%s'",
                       path, post, cookie_path, t, header, url))
    end
    result = ReadFile(path, true)
    return result
end
...
function QMPlugin.GetHttpC(url, cookie_path, t, header)
    ...
    if t == null then t = 30 end
    if header == null then
        os.execute(string.format("curl -o %s -b '%s' --connect-timeout %s '%s'",
                                 path, cookie_path, t, url))
    else
        os.execute(string.format(
                       "curl -o %s -b '%s' --connect-timeout %s -H '%s' '%s'",
                       path, cookie_path, t, header, url))
    end
    result = ReadFile(path, true)
    return result
end
...
function QMPlugin.RegexFind(str, pattern)
    ...
    local iRet, sRet = pcall(function()
        local ret = {}
        for v in string.gmatch(str, pattern) do table.insert(ret, v) end
        return ret
    end)
    if iRet == true then
        return sRet
...
function SQLSelect(DBpath, tbl, field, where, req)
    ...
    local iRet, sRet = pcall(function()
        if where == "" then
            if req == "" then
                return execute(string.format("sqlite3 %s 'select %s from %s'",
                                             DBpath, field, tbl))
            else
                return execute(string.format(
                                   "sqlite3 %s 'select %s from %s %s'", DBpath,
                                   field, tbl, req))
            end
        else
            if req == "" then
                return execute(string.format(
                                   "sqlite3 %s 'select %s from %s where %s'",
                                   DBpath, field, tbl, where))
            else
                return execute(string.format(
                                   "sqlite3 %s 'select %s from %s where %s %s'",
                                   DBpath, field, tbl, where, req))
            end
        end
    end)
    if iRet == true then
...
function SQLInsert(DBpath, tbl, valtbl)
    ...
    local iRet, sRet = pcall(function()
        ...
        local ret = os.execute(string.format(
                                   "sqlite3 %s 'insert into %s(%s) values(%s)'",
                                   DBpath, tbl, key, val))
        if ret then
            return true
        else
...
function QMPlugin.SQLUpdate(DBpath, tbl, valtbl, where)
    ...
    local iRet, sRet = pcall(function()
        ...
        if where == "" then
            ret = os.execute(string.format("sqlite3 %s 'update %s set %s '",
                                           DBpath, tbl, str))
        else
            ret = os.execute(string.format(
                                 "sqlite3 %s 'update %s set %s where %s'",
                                 DBpath, tbl, str, where))
        end
        if ret then
            return true
        else
...
function QMPlugin.SQLDelete(DBpath, tbl, where)
    ...
    local iRet, sRet = pcall(function()
        local ret
        if where == "" then
            ret = os.execute(string.format("sqlite3 %s 'delete from %s '",
                                           DBpath, tbl))
        else
            ret = os.execute(string.format(
                                 "sqlite3 %s 'delete from %s where %s'", DBpath,
                                 tbl, where))
        end
        if ret then
            return true
...
function QMPlugin.LastSMS()
    ...
    local iRet, sRet = pcall(function()
        local dbpath =
                                     "/data/data/com.android.providers.telephony/databases/mmssms.db" -- "/storage/emulated/legacy/mmssms.db"
        return SQLSelect(dbpath, "sms", "body", "",
...
function QMPlugin.GetSMS(number)
    ...
    local iRet, sRet = pcall(function()
        local dbpath =
                                     "/data/data/com.android.providers.telephony/databases/mmssms.db" -- "/sdcard/mmssms.db"
        -- 组合电话号码
...
function QMPlugin.AppendPhone(contact)
    ...
    local iRet, sRet = pcall(function()
        local dbpath =
                                     "/data/data/com.providers.contacts/databases/contacts2.db" -- "/sdcard/contacts2.db"
        local maxID, nickname, phone
        local ret
        if Mount("/data/data") == fale then
            return false
        else
            if ChmodEx(dbpath, 777) == false then return false end
        end
        -- 获取ID最大值
        ...
        for k, v in pairs(contact) do
            nickname = k
            ret = SQLInsert(dbpath, "data",
                            {
                ["data1"] = nickname,
                ["raw_contact_id"] = maxID + 1,
                ["mimetype_id"] = 7
            })
            if v:find("|") then
...
function QMPlugin.IsDisplayChange(x, y, x1, y1, t, delay)
    ...
    while (LuaAuxLib.GetTickCount() - t1) < t * 1000 do
        ...
        intx = LuaAuxLib.FindPicture(x - 10, y - 10, x1 + 10, y1 + 10, PicPath,
                                     "101010", 0, 0.8)
        if intx == -1 then
...
function QMPlugin.Eqv(t1, t2)
    ...
    if type(t1) == "boolean" and type(t2) == "boolean" then
        ...
        for i = 0, 31 do
            ...
            if b1 == b2 then ir = bit32.bor(ir, 2 ^ i) end
        end
        return ir
    end
end
...
function QMPlugin.Coordinate(ScreenX, ScreenY, Time)
    ...
    while data ~= nil do
        ...
        if data ~= nil then
            ...
            if find ~= nil then maxx = find end
            find = nil
            _, _, find = data:find("ABS_MT_POSITION_Y.*max%s+(%d*)")
            if find ~= nil then maxy = find end
        end
    end
...
local function digit_to(n, s)
    ...
    while i >= 0 do
        if n >= (#s) ^ i then
            ...
            if ret ~= "" then ret = ret .. s:sub(1, 1) end
        end
        i = i - 1
    end
    return ret
...
local function h2s(h)
    return (h:gsub("(%x%x)[ ]?",
                   function(w) return string.char(tonumber(w, 16)) end))
end
-- unicode转utf8  [作者：小玮]
function QMPlugin.Unicode2Utf8(us)
    sh_init()
    local u16p = {0xdc00, 0xd800}
    local u16b = 0x3ff
    local u16fx = ""
    local padl = {["0"] = 7, ["1"] = 11, ["11"] = 16, ["111"] = 21}
    local padm = {}
    for k, v in pairs(padl) do padm[v] = k end
    ...
    return (string.gsub(us, "\\[Uu](%x%x%x%x)", function(uc)
        ...
        while #bins < padl[pads] do bins = "0" .. bins end
        local tmp = ""
        if pads ~= "0" then
            tmp = bins
            bins = ""
        ...
        return (string.gsub(string.format("%x", to_digit(pads .. bins, "01")),
                            "(%x%x)",
                            function(w)
...
function QMPlugin.Utf82Unicode(s, upper)
    ...
    local loop2 = string.gsub(loop1, "u/(%x%x*)/", function(w)
        ...
        bi = bi .. tmp
        while string.sub(bi, 1, 1) == "1" do bi = string.sub(bi, 2, -1) end
        wc = to_digit(bi, "01")
        if (wc >= u16id) then
            wc = wc - u16id
            tmp = digit_to(wc, "01")
            ...
            return s2h(upper .. string.format("%4x", high) .. upper ..
                           string.format("%4x", low))
        end
        local h = string.format("%x", wc)
        if (#h) % 2 ~= 0 then h = "0" .. h end
        return s2h(upper .. h)
    end)
    return h2s(loop2)
end
...
function QMPlugin.SecToDay(Sec)
    ...
    local iRet, sRet = pcall(function()
        ...
        if Sec < 0 then return "0天0小时0分0秒" end
        Sec = tonumber(Sec)
        for i = 1, Sec / 60 do
            Min = Min + 1
            if Min == 60 then
...
function QMPlugin.GetDistance(lat1, lng1, lat2, lng2)
    ...
    return math.floor(2 *
                          math.asin(math.sqrt(
                                        math.pow(
                                            math.sin((radLat1 - radLat2) / 2), 2) +
                                            math.cos(radLat1) *
                                            math.cos(radLat2) *
                                            math.pow(
                                                math.sin(
                                                    (rad(lng1) - rad(lng2)) / 2),
                                                2))) * 6378.137 * 10000) / 10000
...
function QMPlugin.CompareTime(Year, Month, Day, Hour, Min, Sec)
    sh_init()
    local NowNetTime = LuaAuxLib.URL_OperationGet(
                           "http://52xiaov.com/getipinfo.php"):match(
                           "秒数%):(.-)</body>")
    if NowNetTime == nil then
        return null
    else
        NowNetTime = tonumber(NowNetTime)
    end
    local Time = {}
    Time.year = Year
    Time.month = Month
...
function QMPlugin.GetNetTime()
    ...
    local iRet, sRet = pcall(function()
        ...
        _, _, _, day, mouth, year, hour, min, sec =
                                     string.find(timetext,
                                                 "Date: (%a+), (%d+) (%a+) (%d+) (%d+):(%d+):(%d+)")
        Time = {
            ["Jan"] = 1,
            ["Feb"] = 2,
        ...
        if year == nil then
            ...
            return string.format("%s/%s/%s %s:%s:%s", year, Time[mouth], day,
                                 tonumber(hour) + 8, min, sec)
        end
    end)
    if iRet == true then
        return sRet
...
function QMPlugin.GetUnixTime(tim)
    local tmpstr = tim .. " "
    local tDate = {}
    _, _, tDate.year, tDate.month, tDate.day, tDate.hour, tDate.min, tDate.sec =
        tmpstr:find("(%d-)%-(%d-)%-(%d-) (%d-):(%d-):(%d-) ")
    if tDate.year then
...
-- 执行 am start 命令  [作者：4Ever]
function QMPlugin.StartActive(package, d)
    os.execute(string.format("am start -n %s -d '%s'", package, d))
end

-- 用今日头条app打开自身新闻  [作者：4Ever]
function QMPlugin.GetToutiaoId(url)

    pattern = "www.toutiao.com/[ai]+(%d+)"
    a, b, result = string.find(url, pattern)
    if result == nil then
        pattern = "www.toutiao.com/group/(%d+)"
        a, b, result = string.find(url, pattern)
    end

    if result == nil then
        pattern = "m.toutiaocdn.com/group/(%d+)"
        a, b, result = string.find(url, pattern)
    end
...
-- 用今日头条app打开自身新闻  [作者：4Ever]
function QMPlugin.StartToutiao(url)
    ...
    local activeFullName =
        "com.ss.android.article.news/com.bytedance.news.schema.AdsAppActivity"
    os.execute(string.format(
                   "am start -n %s -d 'snssdk143://detail?groupid=%s'",
                   activeFullName, result))
end

-- 用今日头条app打开自身新闻  [作者：4Ever]
function QMPlugin.StartToutiaoWithId(newsId)
    
    local activeFullName =
        "com.ss.android.article.news/com.bytedance.news.schema.AdsAppActivity"
    os.execute(string.format(
                   "am start -n %s -d 'snssdk143://detail?groupid=%s'",
                   activeFullName, newsId))
end

-- 用腾讯新闻app打开自身新闻  [作者：4Ever]
function QMPlugin.StartTencent(url)

    pattern = "view.inews.qq.com/a/(.+)%?"
    a, b, result = string.find(url, pattern)

    local activeFullName = "com.tencent.news/.ui.NewsJumpActivity"
    os.execute(string.format(
                   "am start -n %s -d 'qqnews://article_9527?nm=%s&iscomment=1&chlid=news_news_top&articletype=0&&startextras={\"path\":\"a\",\"shareinfo\":\"a|fixed|normal|G|\"}&extinfo='",
                   activeFullName, result))
end

-- 用网易新闻app打开自身新闻  [作者：4Ever]
function QMPlugin.StartNetease(url)

	pattern = ".+/(.+).html"
    a, b, result = string.find(url, pattern)

    local activeFullName =
        "com.netease.newsreader.activity/com.netease.nr.biz.push.newpush.PushActivity"
    os.execute(string.format(
                   "am start -n %s -d 'newsapp://tie/%s/'",
                   activeFullName, result))
end

-- 用抖音app打开自身新闻  [作者：4Ever]
function QMPlugin.StartDouyin(url)

	pattern = "https://www.iesdouyin.com/share/video/(.+)/"
    a, b, result = string.find(url, pattern)

    local activeFullName =
        "com.ss.android.ugc.aweme/.app.AdsAppActivity"
    os.execute(string.format(
                   "am start -n %s -d 'snssdk1128://aweme/detail/%s'",
                   activeFullName, result))
end
...
--
--正则替换
function QMPlugin.RegexReplace(str,pattern,replaced)
	sh_init()

	return string.gsub(str,pattern,replaced)
end 


--正则提取
function QMPlugin.RegexMatch(str,pattern)
	sh_init()

    a, b, result = string.find(str, pattern)
	return result
end 
