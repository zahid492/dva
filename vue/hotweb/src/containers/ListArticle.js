...
const {Search} = Input;
const Timer = {};
const platForms = apiUrl.platForms;
const categorySource = apiUrl.categorySource;
...
class ListArticle extends Component {
    ...
    //发布账号列表
    loadAccounts = () => {
        query.__request('get', apiUrl.GetWebAccountsUrl, {page: 1, size: 10000, platfrom: '', status: 1}, (res) => {
    ...
    changePlatform = (plat) => {
        ...
        if (plat) {
            let categorys = categorySource[plat];
    ...
    changeCategory = (category) => {
        ...
        if (category) {
            ...
            let source = this.state.accountList.filter(ac => ac.platform === platform);
            this.setState({
                category: category,
                accountDatasource: source,
            });
