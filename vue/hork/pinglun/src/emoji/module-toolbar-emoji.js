...
import emojiList from './emoji-lis.js';
...
function fn_emojiPanelInit(panel, quill) {
    fn_emojiElementsToPanel('默认', panel, quill);
    document.querySelector('.filter-默认').classList.add('active');
}
...
function fn_emojiElementsToPanel(type, panel, quill) {
    ...
    result.map(function (emoji) {
        ...
        let output = emoji.value;
        span.innerHTML = output;
        $(span).css({"background-image": `url(${emoji.icon})`})
        ...
        if (customButton) {
            customButton.addEventListener('click', function () {
                ...
                // let emoji_icon_html = makeElement("span", {className: "ico", innerHTML: emoji.value});
