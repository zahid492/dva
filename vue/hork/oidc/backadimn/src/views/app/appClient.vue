    ...
    <el-row class="client-box">
        ...
        <el-col :span="22" :offset="1">
            ...
            <el-table :data="clientList" style="width: 100%" class="client-table">
                ...
                <el-table-column label="重定向Url" width="300">
                    <template slot-scope="scope">
        ...
        methods: {
            ...
            async getClient() {
                try {
                    ...
                    let res = await service({
                        url: "application/" + this.id + "/searchclients",
                        method: "get",
                        params: {
                            Page: this.Page,
                            Size: this.Size,
                        ...
                        try {
                            if (this.flagCM == "0") {
                                clientPass = await service({
                                    url: "application/" + this.id + "/createclient",
                                    method: "post",
                                    data: this.form
                                });
                            }
                            ...
                            if (this.flagCM == "1") {
                                clientPass = await service({
                                    url: "application/" + this.id + "/updateclient",
                                    method: "post",
                                    data: this.form
                                });
                            }

                            if (this.form.allowedgranttypes === "client_credentials" && this.flagCM == "0") {
            ...
            resetSecret(row) {
                ...
                this.$confirm("请注意：重置Secret立即生效，所有使用旧Secret的接口将立即失效。为确保授权系统的正常使用，请尽快更新Secret信息。", '重置Secret', {
                    ...
                    try {
                        let res = await service({
                            url: 'application/' + this.id + "/resetclientsecret",
                            method: "post",
                            data: {
                                applicationid: this.id,
                                id: row.id
