...
import {Input,AutoComplete ,Popconfirm, Modal, message,Select,Tag,BackTop,notification,Radio, Button  } from 'antd';
...
SearchSimilarSentence = (id) =>{
    ...
    _this.setState({
     ...
     query.callData(url).then((res)=>{     
         ...
         {
            let _sentence = res.data.useSentence.replace(/<\/ins>/g,'').replace(/<ins([\s\S]*?)>/g,'');

            _this.setState({ 
                sentenceData:{_id:id,sentence:_sentence,changedSentence:_sentence,originalSentence:res.data.originalSentence,changewordIndex:-1,logs:[]},
                sentenceDataloading:false
            })
         }          
    ...
    //let logs = sentenceObj.logs;     
        ...
        if(sentence == changedSentence)
        {
            if(hide)
            {
                ...
                let logurl = apiUrl.apiseoArticleEditSentenceLogUrl
                let param = 
                  {
                    newArticleId: this.props.match.params.id,
                    oriSentence: sentenceObj.originalSentence,
                    recSenctence: sentence,
                    useSentence: changedSentence
                  }
                ...
                query.setData(url,changedSentence).then(()=>{
                    ...
                    let _sentence = changedSentence.replace(/<\/ins>/g,'').replace(/<ins([\s\S]*?)>/g,'');                  
...
getHitRate = ()=>{
    let _this = this;
    let url = apiUrl.apiseoArticleEditHitRateUrl;
    let OneArticle = this.state.OneArticle;
    _this.setState({
        hitloading:true
    });
    query.setData(url,_this.state.id).then(res=>{
        if(res.code === 200 && res.data)
        {
            let rate = parseFloat(res.data.hitRate);
            if(isNaN(rate))
            {
                rate = 0;
            }
            OneArticle.hitRate =  rate;         
            _this.setState({
                OneArticle,hitloading:false
            });
...
changeChedSentence= (e)=>{
    ...
    let sentenceData =  this.state.sentenceData;
    sentenceData.changedSentence = value;
    this.setState({
        sentenceData
})
}
                            ...
                            <div className="broadcast" id='mainBox'>
                                <div className="top">
                                    ...
                                    <div  style={{display:'inline-block', float:'right',paddingRight:'50px'}}>
                                        <Button type="primary" style = {{marginRight:'30px'}} loading={this.state.hitloading} onClick={this.getHitRate}>
                                            计算飘红率
                                        </Button>                                         
                                        <span style = {{marginRight:'30px'}}>  飘红率：{item && parseInt((item.hitRate + 0.00005) * 10000,10) / 100 + '%' }</span>
                        ...
                        <Modal  title="改编句" destroyOnClose={true} visible={this.state.SentenceHandleVisible} width ={1000}  footer = {null} onCancel={this.hideModal}>                               
                                ...
                                <ul className="gaibianlist">
                                    ...
                                    <li className="item item2">
                                        ...
                                        <TextArea  value= {sentenceData.changedSentence} onChange = {this.changeChedSentence} style={{height:80,marginLeft:'5px',position:'relative',bottom:'20px',zIndex: 1, borderWidth: 0}}  />

                                        <img src="/img/gaibian1.png" alt="" className="gaibian1" />
                                    </li>
                                </ul>
                                {/* <div style={{padding:'8px',display: 'none'}}>
                                ...
                                <div className="btn-box" style={{marginTop:'15px'}}>
                                    <button className="btn-fh" style={{marginRight:'30px'}} onClick={()=>this.sentenceToNext(true)}>下一条</button>
                                    <button className="btn-bc" style={{marginRight:'30px'}} onClick={()=>this.sentenceSubmit(true)}>保存</button>
