...
// 0 登录
function* sendLogin(action, apiFn) { 
    ...
    if (response) {
        ...
        if(response.accesstoken)
        {
            ...
            yield call(fetchEntity.bind(null, actions.articleTasks, newsApi.UserArticleTasksGet, param));
        }
...
export const sendAccountLogin = sendLogin.bind(null, actions.login, newsApi.loginApi);

function* login() {
    while (true) {
        yield call(sendAccountLogin)
    }
}

// 0 退出
export const sendAccountLoginOut = sendLogin.bind(null, actions.loginout, newsApi.logoutApi);

function* loginout() {
    while (true) {
        yield call(sendAccountLoginOut)
    }
}

// 0 accessLogin
export const sendAccessLogin = sendLogin.bind(null, actions.accessLogin, newsApi.accessLoginApi);

function* accessLogin() {
    while (true) {
        yield call(sendAccessLogin)
    }
}
...
// 1 添加借势传播
function* addTrendGen(action, apiFn) {
    const opt = yield take(actions.LOAD_AddTrend);
    const {response, error} = yield call(apiFn, opt);
...
export const sendAddTrendGen = addTrendGen.bind(null, actions.addTrend, newsApi.addTrendApi);

function* sendAddTrend() {
    while (true) {
        yield call(sendAddTrendGen)
    }
}

// 2 查询借势传播
function* loadTrend2() {
    const opt = yield take(actions.LOAD_Trend2);

    yield call(fetchEntity.bind(null, actions.Trend2, newsApi.getTrend2, opt))
}

function* watchTrend2() {
    while (true) {
        yield call(loadTrend2)
    }
}
...
function* loadTags() {
    ...
    yield call(fetchEntity.bind(null, actions.Tags, newsApi.tagsApi));
...
function* loadKeyInfo() {
    let opt = yield take(actions.LOAD_KeyInfo);
    yield call(fetchEntity.bind(null, actions.keyInfo, newsApi.getKeyInfo, opt));
}

function* watchloadKeyInfo() {
    while (true) {
        yield call(loadKeyInfo)
    }
}

function* sendKeyInfo(action, apiFn) {
    const opt = yield take(actions.UpKeyInfoTypes["REQUEST"]);
    const {response, error} = yield call(apiFn, opt);
...
export const sendKeyInfoAdd = sendKeyInfo.bind(null, actions.upKeyInfo, newsApi.updateKeyInfo);

function* watchSendKeyInfo() {
    while (true) {
        yield call(sendKeyInfoAdd)
    }
}



// 3 加载借势传播列表， 不分页返回30条，但需要过滤显示 加载 trend3
function* loadNewTrend3s() {
    let opt = yield take(actions.LOAD_NewTrend3s);

    yield call(fetchEntity.bind(null, actions.newTrend3s, newsApi.getNewTrend3s, opt));
}

function* watchLoadNewTrend3s() {
    while (true) {
        yield call(loadNewTrend3s)
    }
}


// 3 生成借势传播列表 trend3->4
function* genNewTrend4() {
    const opt = yield take(actions.genNewTrend4s.request().type);
    const {response, error} = yield call(newsApi.genNewTrend4s, opt);

    if (response) {
        yield put(actions.genNewTrend4s.success(opt, response));

    } else {
        yield put(actions.genNewTrend4s.failure(opt, error))
    }
}

function* watchGenNewTrend4s() {
    while (true) {
        yield call(genNewTrend4)
    }
}


// 1 一键改编
function* AdaptGen(action, apiFn) {
    const opt = yield take(actions.GenAdaptTypes["REQUEST"]);
    const {response, error} = yield call(apiFn, opt);
...
export const sendAdaptGen = AdaptGen.bind(null, actions.genAdapt, newsApi.adaptArticle);

function* sendAdapt() {
    while (true) {
        yield call(sendAdaptGen)
    }
}

function* loadTrendList() {
    let opt = yield take(actions.LOAD_NewTrend4s);

    yield call(fetchEntity.bind(null, actions.newTrend4s, newsApi.trendList, opt));
}

function* watchLoadTrendList() {
    while (true) {
        yield call(loadTrendList)
    }
}
