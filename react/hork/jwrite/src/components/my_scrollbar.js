export default (function (win, dom) {
	...
	MyScrollBar.prototype.initEvent = function () {
		...
		if ( this.bYBar ) {
			...
			// 禁止默认拖动事件
			this.oYBar.ondrag = function (evt) {
				var e = evt || win.event;
				e ? e.preventDefault() : evt.returnValue = false;
			}
		}
		...
		if ( this.bXBar ) {
			...
			// 禁止默认拖动事件
			this.oXBar.ondrag = function (evt) {
				var e = evt || win.event;
				e ? e.preventDefault() : evt.returnValue = false;
			}
		}	
	...
	function getPosition (obj, goal) {
		...
		if ( obj.parentNode != goal ) {
			obj = getPosition(obj.parentNode, goal);
			oPos.top += obj.top;
			oPos.left += obj.left;
		} else {
			return oPos;
